...
class Program{
    ...
    switch (op)
    {
        case "1": ListarTodosLosExamenes(db); break;
        case "2": ReportePorAlumno(db); break;
        case "3": RankingMejoresAlumnos(db); break;
        case "4": InformePorPregunta(db); break;
        case "5": return;
    ...
    static void ListarTodosLosExamenes(DatosContexto db)
    {
        ...
        {
            Console.WriteLine($"{e.Fecha:g} | Alumno: {e.Alumno} | Correctas: {e.Correctas}/{e.Total} | Nota: {e.Nota}");
        }
    }
    static void ReportePorAlumno(DatosContexto db)
{
    Console.Write("Ingrese el nombre del alumno: ");
    var nombre = Console.ReadLine()!;
    var examenes = db.Resultados
                     .Where(x => x.Alumno == nombre)
                     .OrderByDescending(x => x.Fecha)
                     .ToList();

    if (!examenes.Any())
    {
        Console.WriteLine("No hay exámenes para ese alumno.");
        return;
    }

    foreach (var e in examenes)
    {
        Console.WriteLine($"{e.Fecha:g} | Correctas: {e.Correctas}/{e.Total} | Nota: {e.Nota}");
    }
}

static void RankingMejoresAlumnos(DatosContexto db)
{
    var ranking = db.Resultados
        .GroupBy(x => x.Alumno)
        .Select(g => new { Alumno = g.Key, MejorNota = g.Max(e => e.Nota) })
        .OrderByDescending(x => x.MejorNota)
        .Take(10)
        .ToList();

    Console.WriteLine("Ranking alumnos con mejor nota:");
